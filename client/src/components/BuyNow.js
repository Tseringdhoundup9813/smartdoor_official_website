...
export default function BuyNow() {
    ...
    async function totalproductamount() {
      try {
        const totalamount = await axios.get(
          `https://node.smartdoors.com.np/totalamount/${localStorage.getItem("user_id")}`
        );
  ...
  async function placeOrder() {
        ...
        try {
          const payorder = await axios.post(`https://node.smartdoors.com.np/orders/${localStorage.getItem("user_id")}`,
  ...
  async function productlistshow() {
    try {
      const product = await axios.get(
        `https://node.smartdoors.com.np/showCartlist/${localStorage.getItem("user_id")}`
      );
