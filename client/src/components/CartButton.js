...
function CartButton(props) {
    ...
    async function AddQt(quantity){
        ...
        try{
            const product = await axios.post(`https://node.smartdoors.com.np/addqt`,{product_id:props.product_id,user_id:props.userKey})
    ...
    async function MinQt(quantity){
        if(qt > 1){
            try{
                const product = await axios.post(`https://node.smartdoors.com.np/minqt`,{product_id:props.product_id,user_id:props.userKey})
     ...
     useEffect(()=>{
        ...
        async function getQuantity(){
            try{
                const product = await axios.get(`https://node.smartdoors.com.np/quantity/${props.product_id}/${props.userKey}`)
