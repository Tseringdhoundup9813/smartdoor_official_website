    ...
    async function getQuantity() {
      try {
        const product = await axios.get(
          `https://node.smartdoors.com.np/quantity/${props.product_id}/${props.userKey}`
        );
  ...
  return (
    <div>
      <div className="cart-product-incerase align-items-center ">
        <div className="cart-product-minus" onClick={cartminus}>
          <i class="fa-solid fa-minus"></i>
        </div>
        <div className="cart-product-number">{qt} </div>

        <div className="cart-product-plus" onClick={cartplus}>
