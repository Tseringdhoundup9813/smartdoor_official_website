...
function Navbar() {
  ...
  async function totalcartlistcount() {
    try {
      let carttotal = await axios.get(
        `https://node.smartdoors.com.np/totalcartlistcount/${localStorage.getItem(
          "user_id"
        )}`
      );
  ...
  async function VerifyEmailSend() {
    ...
    const verifySend = await axios.get(
      `verifyEmail/${localStorage.getItem("user_id")}`
    );
    ...
    <div>
      ...
      {Email_verify_message ? (
        <div className="email-verification-notification">
          <div>{Email_verify_message}</div>
          <div className="email-red">Send Email Again</div>
        </div>
      ) : JSON.parse(localStorage.getItem("email_verify")) ||
        JSON.parse(localStorage.getItem("email_verify")) == null ? (
        ""
      ) : (
        <div className="email-verification-notification">
          <div>
            {localStorage.getItem("username")} You need to Verify your Email
          </div>
          <div className="email-red" onClick={VerifyEmailSend}>
      ...
      <nav className="navbar navbar-expand-md bg-body-tertiary ">
        <div className="container-fluid navbar-container">
          ...
          <div className="two-icon order-md-3">
            ...
            {token ? (
              <div id="logout">
                <div className="logout-username " onClick={clickedUser}>
              ...
              <div
                className="heart-icon d-flex align-items-center justify-content-center text-capitalize nav-item"
                id="logout"
                onClick={clickLogin}
              >
                login{" "}
            ...
            <div className="cart-icon fs-4">
              <p className="totalcartlistCount">{totalCartList}</p>
              <NavLink to="/product-cart" className=" nav-link cart-icon">
            ...
            <ul className="navbar-nav navbar-ul">
              ...
              <li className="nav-item">
                <NavLink
                  to="/about/our-vision"
                  onClick={navItem}
                  className=" dropdown-item nav-link text-capitalize"
                >
                  About Us
                </NavLink>
              </li>
