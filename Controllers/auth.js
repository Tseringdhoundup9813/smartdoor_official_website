...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    if (isEmail) {
      ...
      sendToken(
        "sucessfully Singup && Email send to verify",
        user,
        user.username,
        user._id,
        false,
      ...
      const url = `https://node.smartdoors.com.np/user/${user._id}/verify/${token.token}`;
...
exports.VerifyEmail = async (req, res) => {
  ...
  try {
    ...
    const url = `https://node.smartdoors.com.np/user/${id}/verify/${token.token}`;
    await Email(user_Email[0].email, "Email Verification", url);
    res.status(200).json({
      message: "Email Verification Send to your gmail " + user_Email[0].email,
    });
  } catch (err) {
    console.log(err);
...
exports.forgetpassword = async (req, res, next) => {
  ...
  try {
    ...
    await user.save();
    const resetUrl = `https://node.smartdoors.com.np/passwordreset/${resetToken}`;
...
const sendToken = (msg, user, username, id, email_verify, statusCode, res) => {
  ...
  res
    .status(statusCode)
    .json({ message: msg, success: true, token, username, id, email_verify });
};
