...
const CustomerMessage = require("../Models/CustomerQuery");
...
exports.ProductUpload = async (req, res) => {
  ...
  const { name, description, price, discount, colors, categories, size } =
    req.body;
  ...
  try {
    if (req.files) {
      for (var a = 0; a < imgdata.length; a++) {
        if (a < 2) {
          path.push(
            url + "/Uploads/" + imgdata[a].filename.split(" ").join("")
          );
        }
      }
    }
...
exports.ProductAll = async (req, res) => {
  try {
    ...
    const colors =
      req.query.colors === "all"
        ? ["rose wood", "andrateak"]
        : req.query.colors;
    const categories =
      req.query.category === "all"
        ? ["3D DOORS", "DOUBLE DOORS", "CANADA DOORS", "MEMBRANE DOORS"]
        : req.query.category;
    const sizes =
      req.query.size === "all"
        ? ["80-32", "80-26", "75-26", "72-26", "80-38", "DD80-19", "DD80-22"]
        : req.query.size;
...
exports.AddToCart = async (req, res) => {
  ...
  try {
    const cartlistExist = await AddToCart.find({
      userId: userId,
    ...
    if (cartlistExist.length < 1) {
      ...
      if (product[0].discount > 0) {
        ...
        const addMainProduct = await AddToCart.findOneAndUpdate(
          { userId: userId, productId: product_id },
          { mainproduct: product }
        );
      } else {
      ...
      res
        .status(200)
        .json({ data: addtocart, success: true, cartexist: false });
    } else {
      const addtocart = await AddToCart.findOneAndUpdate(
        { userId: userId, productId: product_id },
        { $inc: { quantity: 1 } }
      );
      const product = await productModel.find({ _id: product_id });
...
exports.GetProductCart = async (req, res) => {
  try {
    ...
    const cartlist = await productModel.find(
      { _id: cartarray },
      {},
      { sort: { _id: 1 } }
    );

    const quantity = await AddToCart.find(
      { userId: user_id },
      {},
      { sort: { productId: 1 } }
    );
...
exports.addQt = async (req, res) => {
  ...
  try {
    const rateChange = await AddToCart.findOneAndUpdate(
    ...
    if (P_discount > 0) {
      ...
      const setTotalamount = await AddToCart.findOneAndUpdate(
        { userId: user_id, productId: product_id },
        { amount: totalamount }
      );
      const get_total_amount = await AddToCart.find({
        userId: user_id,
        productId: product_id,
      });
      res.status(200).json({
...
exports.deletecartlist = async (req, res) => {
  ...
  try {
    const deletecart = await AddToCart.findOneAndDelete({
      userId: user_id,
      productId: product_id,
    });
...
exports.totalcartlistcount = async (req, res) => {
  try {
    if (req.params.user_id !== undefined) {
      const totalcartlist = await AddToCart.find({
...
exports.BuyNowProduct = async (req, res) => {
  try {
    const user_product_exits = await BuyNowProduct.find({
      userId: req.params.user_id,
    });
    ...
    if (user_product_exits.length < 1) {
      ...
      await BuyNowProduct.findOneAndUpdate({
        userId: req.params.user_id,
        productId: req.params.product_id,
      });
    }
...
exports.Order = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ order_id: createorder._id, totalamount: totalamount });
  } catch (err) {
    console.log(err);
  }
};
...
exports.khaltidata = async (req, res) => {
  ...
  try {
    ...
    const savepidx = await Order.findByIdAndUpdate(
      { _id: order_id },
      { pidx: pidx }
    );
...
exports.OrderTracking = async (req, res) => {
  ...
  try {
    const user_order = await Order.find({
      userId: req.params.user_id,
      confirm: true,
    });
...
exports.AddCategoryName = async (req, res) => {
  ...
  try {
    if (categoryexist.length < 1) {
      ...
      const category_name = await AddCategory.findByIdAndUpdate(
        { _id: categoryexist[0]._id },
        { $push: { category_name: name } }
      );
    }
    res.status(200).json({ success: true });
  } catch (err) {
...
exports.Contact = async (req, res) => {
  ...
  try {
    const createquery = await CustomerMessage.create({
      name,
      email,
      phone,
      message,
...
exports.CustomerMessageDelete = async (req, res) => {
  console.log(req.params);
  try {
    const messagedelete = await CustomerMessage.deleteOne({
      _id: req.params.id,
    });
...
exports.CustomerMessageCheck = async (req, res) => {
  console.log(req.params);
  try {
    const updatemessage = await CustomerMessage.findByIdAndUpdate(
      { _id: req.params.id },
      { solve: true }
    );
